...
int main(int argc, char *argv[])
{
    ...
    Mario = SDL_LoadBMP("tetris_rectangle.bmp"); //Chargement bmp Mario
    SDL_SetColorKey(Mario, SDL_SRCCOLORKEY, SDL_MapRGB(Mario->format, 255, 255, 255));

    positionbrick.x = ecran->w / 2 - Mario->w / 2; //Mario placé au centre  (largeur  ecran)/2 - (largeur Mario)/2
    positionbrick.y = ecran->h / 2 - Mario->h / 2; //Mario placé au centre  (hauteur  ecran)/2 - (hauteur Mario)/2
    ...
    while (continuer)
    {
        ...
        SDL_BlitSurface(Mario, NULL, ecran, &positionbrick);//Blit fde Mario sur l'écran
        SDL_Flip(ecran);
    }

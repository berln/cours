...
Sommet * Parcours_Profondeur_Trans(MatAdja A, Sommet *S){
     ...
     Sommet * tmp = (Sommet*)malloc(A->nb_sommets*sizeof(Sommet));
      
     int i,j=0;
     int date= 0;
 
     for(i=0;i<A->nb_sommets;i++){
           tmp[i].numero=S[i].numero;
           tmp[i].date_debut=0;
           tmp[i].date_fin=0;
           tmp[i].CompConnexe=0;
     }
     ...
     for(i=0;i<A->nb_sommets;i++){
           ...
           if(tmp[i].date_debut==0){
                profondeur(Transpose(A), tmp, &date, tmp[i].numero);
               j++;
           }
           S[i].CompConnexe=j;
     }
...
int CompFortementConnexes(MatAdja A, int * B){ /* Retourne les CFC de la matrice d'adjascence A*/
     int i,nb=0;
     Sommet *S;
     S = Parcours_Pronfondeur(A); /*Notation des date de d\303\251but et date de fin de S*/
     S = Parcours_Profondeur_Trans(A,Trier(S,A->nb_sommets)); /*Notation des date de d\303\251but et date de fin de S transpos\303\251e*/
     for(i=0;i<A->nb_sommets;i++){ /* Insere dans B tous les groupes de composantes connexes de S transpos\303\251, chaque groupe forme une CFC*/
           B[S[i].numero]=S[i].CompConnexe;    
           if(nb < S[i].CompConnexe){
              nb =S[i].CompConnexe;
           }
     }
     return nb; /* Retourne le nombre de CFC*/
